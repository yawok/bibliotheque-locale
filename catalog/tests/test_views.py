...
class BookListViewTest(TestCase):
    ...
    def setUpTestData(cls):
        no_of_books = 15
        test_author = Author.objects.create(first_name="Kenneth", last_name="Obeng")
...
class BookDetailViewTest(TestCase):
    ...
    def test_view_url_is_accessible_by_name(self):
        response = self.client.get(
            reverse("book-detail", kwargs={"pk": self.test_book.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def setUpTestData(cls):
        ...
        for author in range(no_of_authors):
            Author.objects.create(
                first_name=f"Author{author}", last_name=f"Green{author}"
            )
...
class AuthorDetailViewTest(TestCase):
    def setUp(self):
        ...
        self.test_author = Author.objects.create(
            first_name="Kenneth", last_name="Obeng"
        )

    def test_view_url_is_accessible_by_name(self):
        response = self.client.get(
            reverse("author-detail", kwargs={"pk": self.test_author.id})
        )
        self.assertEqual(response.status_code, 200)
...
class LoanedBookInstanceByUserListViewTest(TestCase):
    def setUp(self):
        test_user1 = User.objects.create_user(
            username="testuser1", password="adamu1234"
        )
        test_user2 = User.objects.create_user(
            username="testuser2", password="adamu1234"
        )
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirect_to_all_borrowed_books_on_success(self):
        ...
        response = self.client.post(
            reverse("renew-book-librarian", kwargs={"pk": self.test_bookinstance2.pk}),
            {"due_back": date_2_weeks_in_future},
        )
    ...
    def test_invalid_renewal_date_past(self):
        ...
        response = self.client.post(
            reverse("renew-book-librarian", kwargs={"pk": self.test_bookinstance2.pk}),
            {"due_back": date_3_weeks_in_past},
        )
        self.assertTrue(response, 200)
        self.assertFormError(
            response, "form", "due_back", "Invalid date - renewal date in the past"
        )
    ...
    def test_invalid_renewal_date_future(self):
        ...
        response = self.client.post(
            reverse("renew-book-librarian", kwargs={"pk": self.test_bookinstance2.pk}),
            {"due_back": date_7_weeks_in_future},
        )
        self.assertTrue(response, 200)
        self.assertFormError(
            response,
            "form",
            "due_back",
            ["Invalid date - renewal date more than 4 weeks away."],
        )
...
class AuthorUpdateViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse("author-update", kwargs={"pk": self.test_author.id})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith, "/accounts/login/")
    ...
    def test_form_contains_correct_author_details(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["form"].initial["first_name"], self.test_author.first_name
        )
...
class AuthorDeleteViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse("author-delete", kwargs={"pk": self.test_author.id})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith, "/accounts/login/")
...
class BookUpdateViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse("author-update", kwargs={"pk": self.test_book.id})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith, "/accounts/login/")
    ...
    def test_forbidden_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username="testuser1", password="adamu1234")
        response = self.client.get(
            reverse("book-update", kwargs={"pk": self.test_book.id})
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_form_contains_correct_author_details(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["form"].initial["title"], self.test_book.title
        )
...
class BookDeleteViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse("book-delete", kwargs={"pk": self.test_book.id})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith, "/accounts/login/")
